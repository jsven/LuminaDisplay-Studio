...
- `src/stage/scene-layout.config.js`: 场景布局配置文件，带中文注释，可直接微调每个场景里相机、显示器、主机设备和桌面物品的位置关系
...
- 以后优先修改 `src/stage/scene-layout.config.js`
- 这份配置文件已经把各个场景的相机、显示器、主机设备、桌面小物和房间大件的位置参数收口，并附了中文注释
- 如果只是想调“谁离谁近一点、朝向偏多少、桌面上放哪几样东西”，通常不需要改渲染代码
